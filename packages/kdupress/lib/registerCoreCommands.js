...
const { chalk } = require('@kdupress/shared-utils')
const envinfo = require('envinfo')

const { dev, build, eject } = require('@kdupress/core')
const { path, logger, env } = require('@kdupress/shared-utils')
...
module.exports = function (cli, options) {
  ...
  cli
    ...
    .option('-d, --dest <dest>', 'specify build output dir (default: .kdupress/dist)')
    .option('-t, --temp <temp>', 'set the directory of the temporary file')
    .option('-c, --cache [cache]', 'set the directory of cache')
  ...
  cli
    .command('eject [targetDir]', 'copy the default theme into .kdupress/theme for customization.')
      ...
      envinfo.run(
        {
          ...
          npmGlobalPackages: ['kdupress'],
          npmPackages: [
            'kdupress',
            '@kdupress/core',
            '@kdupress/theme-default'
          ]
        },
        {
          showNotFound: true,
