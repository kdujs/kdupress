...
const { createApp } = require('@kdupress/core')
const { path, logger, globby, chalk } = require('@kdupress/shared-utils')
...
// When user type `kdupress [customCommand] --help`,
// KduPress will try to detect where user to place docs.
...
async function inferUserDocsDirectory (cwd) {
  ...
  if (siteConfigPath) {
    return path.resolve(
      cwd,
      siteConfigPath.replace('.kdupress/config.js', '')
    )
  }
  return null
}
...
function registerUnknownCommands (cli, options) {
  cli.on('command:*', async () => {
    ...
    if (inferredUserDocsDirectory && sourceDir !== inferredUserDocsDirectory) {
      ...
      console.log()
      logger.tip(`Did you miss to specify the target docs dir? e.g. ${chalk.cyan(`kdupress ${commandName} [targetDir]`)}.`)
      logger.tip(`A custom command registered by a plugin requires KduPress to locate your site configuration like ${chalk.cyan('kdupress dev')} or ${chalk.cyan('kdupress build')}.`)
      console.log()
      process.exit(1)
    }
