...
const { fs, path, hash, parseFrontmatter, inferTitle, extractHeaders } = require('@kdupress/shared-utils')
const LRU = require('lru-cache')
const md = require('@kdupress/markdown')
...
module.exports = function (src) {
  ...
  // kdu-loader, and will be applied on the same file multiple times when
  // selecting the individual blocks.
  const file = this.resourcePath
  const key = hash(file + src)
  const cached = cache.get(key)
  if (cached && (isProd || /\?kdu/.test(this.resourceQuery))) {
    return cached
  }
