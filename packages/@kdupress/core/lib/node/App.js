...
module.exports = class App {
  ...
  constructor (options = {}) {
    ...
    this.kdupressDir = path.resolve(this.sourceDir, '.kdupress')
  ...
  async resolveConfigAndInitialize () {
    if (this.options.siteConfig) {
      ...
      let siteConfig = loadConfig(this.kdupressDir)
    ...
    this.outDir = rawOutDir
      ...
      : require('path').resolve(this.sourceDir, '.kdupress/dist')
  ...
  applyInternalPlugins () {
    ...
    // internl core plugins
      ...
      .use('@kdupress/last-updated', !!shouldUseLastUpdated)
      .use('@kdupress/register-components', {
        componentsDir: [
          path.resolve(this.sourceDir, '.kdupress/components'),
  ...
  applyUserPlugins () {
    ...
    this.pluginAPI
      ...
      .use(this.themeAPI.kdupressPlugin)
      .use(Object.assign({}, this.siteConfig, { name: '@kdupress/internal-site-config' }))
  }
   ...
   *   2. `dev.html` located at .kdupress/templates
  ...
  resolveGlobalLayout () {
    this.globalLayout = this.resolveCommonAgreementFilePath(
      'globalLayout',
      {
        defaultValue: this.getLibFilePath('client/components/GlobalLayout.kdu'),
        siteAgreement: `components/GlobalLayout.kdu`,
        themeAgreement: `layouts/GlobalLayout.kdu`
      }
    )
   ...
   * @param {string} siteAgreement a relative path to kdupress dir
  ...
  async resolvePages () {
    // resolve pageFiles
    const patterns = this.siteConfig.patterns ? this.siteConfig.patterns : ['**/*.md', '**/*.kdu']
    patterns.push('!.kdupress', '!node_modules')

    if (this.siteConfig.dest) {
      // #654 exclude dest folder when dest dir was set in
      // sourceDir but not in '.kdupress'
  ...
  resolveSiteAgreementFile (filepath) {
    return path.resolve(this.kdupressDir, filepath)
  }
