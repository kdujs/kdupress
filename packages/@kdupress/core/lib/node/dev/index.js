...
const { frontmatterEmitter } = require('@kdupress/markdown-loader')
const webpack = require('webpack')
const chokidar = require('chokidar')

const { path, fs, logger } = require('@kdupress/shared-utils')
...
module.exports = class DevProcess extends EventEmitter {
  ...
  watchSourceFiles () {
    ...
    this.pagesWatcher = chokidar.watch([
      '**/*.md',
      '.kdupress/components/**/*.kdu'
    ], {
      cwd: this.context.sourceDir,
      ignored: ['.kdupress/**/*.md', 'node_modules'],
      ignoreInitial: true
    })
  ...
  prepareWebpackConfig () {
    ...
    config
      ...
      .use(require('kdupress-html-webpack-plugin'), [{
  ...
  createServer () {
    const contentBase = path.resolve(this.context.sourceDir, '.kdupress/public')
