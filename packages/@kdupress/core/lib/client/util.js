...
/**
 * This method was for securely getting Kdu component when components
...
const globalComponentsGetter = name => Kdu.component(name)
...
export function getKduComponent (name) {
...
export function ensureAsyncComponentsLoaded (...names) {
  return Promise.all(names.filter(v => v).map(async (name) => {
    if (!getKduComponent(name) && getAsyncComponent(name)) {
      const comp = await getAsyncComponent(name)()
      Kdu.component(name, comp.default)
    }
  }))
}

/**
 * Inject option to Kdu SFC
...
/**
 * Set global info in `window.__KDUPRESS__` for debugging.
 *
 * @param {string}key
 * @param {any} value
 */
export function setGlobalInfo (key, value) {
  if (typeof window === 'undefined' || !window.__KDUPRESS__) {
    return
  }
  window.__KDUPRESS__[key] = value
}
