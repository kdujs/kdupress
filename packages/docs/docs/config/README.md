...
- Type: `string`
- Default: `/path/to/@kdupress/core/.temp`
...
- Type: `string`
- Default: `.kdupress/dist`

Specify the output directory for `kdupress build`. If a relative path is specified, it will be resolved based on `process.cwd()`.
...
KduPress uses [cache-loader](https://github.com/webpack-contrib/cache-loader) by default to greatly speed up the compilation of webpack.
...
You can watch any file if you want. File changes will trigger `kdupress` rebuilding and real-time updates.
...
To apply simple overrides to the styling of the [default preset](https://github.com/kdujs/kdupress/blob/main/packages/@kdupress/core/lib/client/style/config.styl) or define some variables to use later, you can create a `.kdupress/styles/palette.styl` file.
...
KduPress provides a convenient way to add extra styles. You can create a `.kdupress/styles/index.styl` file for that. This is a [Stylus](http://stylus-lang.com/) file but you can use normal CSS syntax as well.
...
- Type: `Function`
- Default: [source](https://github.com/kdujs/kdupress/tree/main/packages/@kdupress/shared-utils/src/slugify.ts)
...
You can install any markdown-it plugins through `markdown.plugins` option. It’s similar with [using KduPress plugins](../plugin/using-a-plugin.html#using-a-plugin). You can either use Babel style or object style. The `markdown-it-` prefix is optional and can omit in the list.
...
While preparing the page, headers are extracted from the Markdown file and stored in `this.$page.headers`. By default, KduPress will extract `h2` and `h3` elements for you. You can override the headers it pulls out in your `markdown` options.
...
KduPress comes with built-in webpack config for the CSS pre-processors listed below. For more information on installation these or pre-processors without built-in support, see [Using Pre-Processors](../guide/using-kdu.md#using-pre-processors) for more information.
:::
...
Set to `true` if you are only targeting everblue browsers. This will disable ES5 transpilation and polyfills for IE, and result in faster builds and smaller files.
