...
- title: Performant
  details: KduPress generates pre-rendered static HTML for each page, and runs as an SPA once a page is loaded.
    ...
    nav: [
      {
        ...
        items: [
          { text: 'Vietnamese', link: '/language/vietnamese/' },
...
KduPress checks each sidebar config from top to bottom. If the fallback configuration was first, KduPress would incorrectly match `/foo/` or `/bar/four.html` because they both start with `/`.
:::
...
- [@kdupress/plugin-last-updated](../plugin/official/plugin-last-updated.md)
...
This will render `.kdupress/components/SpecialLayout.kdu` for the given page.
...
You can copy the default theme source code into `.kdupress/theme` to fully customize the theme using the `kdupress eject [targetDir]` command. If you didn’t install KduPress globally, run `./node_modules/.bin/kdupress eject`.

::: warning
Once you eject, you are on your own and **won’t** be receiving future updates or bugfixes to the default theme even if you upgrade KduPress.
:::
