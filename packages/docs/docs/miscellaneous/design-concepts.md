...
# Design Concepts of KduPress 1.x

The design concepts of KduPress 1.x are mainly reflected in the following aspects:
...
::: tip
The options supported by `.kdupress/config.js` are actually based on the plugin options and add some specific options.
:::
...
::: tip
As with `.kdupress/config.js`, the options supported by `theme/index.js` are based on the plugin options and add some specific options. Using a graph to express their relationship:
...
<svg viewBox="0 0 2806 912" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        ...
        <text id=".kdupress/-config.js" font-family="ArialMT, Arial" font-size="60" font-weight="normal" fill="#FFFFFF">
            <tspan x="901.101562" y="436">.kdupress/</tspan>
...
In KduPress, you have the ability to apply some plugins in a plugin:
...
KduPress 1.0 begin to introduce some conventions to reduce the user’s excessive configuration pressure, the most intuitive manifestation of this is the conventions for the [document directory structure](../guide/directory-structure.md) and the [theme directory structure](../theme/writing-a-theme.md#directory-structure).
...
::: warning Note
When customizing `templates/ssr.html`, or `templates/dev.html`, it’s best to edit it on the basis of the [default template files](https://github.com/kdujs/kdupress/blob/main/packages/%40kdupress/core/lib/app/index.dev.html), otherwise it may cause a build failure.
:::
...
With the goal of decoupling, we were able to separate KduPress into the following two libraries by introducing monorepo:

- [@kdupress/core](https://github.com/kdujs/kdupress/tree/main/packages/@kdupress/core)：Including the core implementation of `dev`, `build` and `Plugin API`;
- [@kdupress/theme-default](https://github.com/kdujs/kdupress/tree/main/packages/@kdupress/theme-default)：The default theme you see now.

Of course, for most users, you don’t need to worry about these three libraries. The [KduPress](https://www.npmjs.com/search?q=kdupress) package has already assembled them together.
